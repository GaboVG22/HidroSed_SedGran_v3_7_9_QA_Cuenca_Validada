...
st.set_page_config(page_title="HidroSed Maestra Integrada · v3.7.9", page_icon="🌊", layout="wide")
...
st.markdown(
    dashboard_header_html(
        "HidroSed Maestra Integrada · v3.7.9",
        "Formato visual tipo tablero profesional: flujo lateral, tarjetas KPI, secciones compuestas y QA estricto de punto de control, cauce DEM y cuenca no truncada."
    ),
    unsafe_allow_html=True,
)
...
with tabs[2]:
    ...
    if not has("dem_path") or not has("control_point"):
        ...
        st.info(
            "QA v3.7.9 activo: la app corrige automáticamente el punto hacia el cauce DEM, "
            "evalúa candidatos de salida, descarta cuencas truncadas por borde/NoData y solo habilita la cuenca oficial si pasa controles mínimos."
        )
        ...
        if st.button("Delimitar, corregir punto y validar cuenca", type="primary"):
            ...
            try:
                ...
                if bool(result.metrics.get("cuenca_validada")):
                    ...
                    st.success("Cuenca VALIDADA: punto corregido al cauce DEM y controles mínimos aprobados.")
                else:
                    st.error("Cuenca NO VALIDADA: se muestra diagnóstico, pero no se habilita como cuenca oficial ni se entrega KMZ/KML validado.")
            except Exception as exc:
                st.error(str(exc))
        ...
        if isinstance(m, dict) and m:
            ...
            if bool(m.get("cuenca_validada")) and has("basin_kmz") and has("basin_kml"):
                ...
                st.info("Descarga de cuenca bloqueada: solo se habilita cuando el diagnóstico indique Estado QA = VALIDADA.")
...
"""
    ...
    with audit_tabs[6]:
        ...
        md = technical_markdown_report(
            "Informe de Auditoría General HidroSed Maestra Integrada v3.7.9",
        ...
        try:
            docxb = docx_report_bytes("Informe de Auditoría General HidroSed Maestra Integrada v3.7.9", st.session_state.get("aud_external_audit_df", pd.DataFrame()), st.session_state.get("aud_score_df", pd.DataFrame()), st.session_state.get("aud_adoption_df", pd.DataFrame()))
        ...
        try:
            pdfb = pdf_report_bytes("Informe de Auditoría General HidroSed Maestra Integrada v3.7.9", st.session_state.get("aud_external_audit_df", pd.DataFrame()), st.session_state.get("aud_score_df", pd.DataFrame()), st.session_state.get("aud_adoption_df", pd.DataFrame()))
